...
void LevelScene::onButtonPressed(const UserEvent& e)
{
    ...
    else if(*buttonType == "bet" && betBtn->isVisible() && (bank->getTotal() > 0))
    { 
...
void LevelScene::bet()
{
    ...
    if ((playerHands[currentHand].getValue() == 9) || (playerHands[currentHand].getValue() == 10) || (playerHands[currentHand].getValue() == 11)) dd = true;
...
void LevelScene::dealDealer()
{
    ...
    addGameObject(card, 3);
    dealerHand.addCard(card);
    if (playerHands[currentHand].getValue() > 21 || (dealerHand.getValue() <= 21 && dealerHand.getValue() > playerHands[currentHand].getValue())) {
        LOG_INFO("Dealer wins!\n");
        startGame();
    }
